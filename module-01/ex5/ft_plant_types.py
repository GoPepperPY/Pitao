...
class Flower(Plant):
    def __init__(self, name: str, height: float, age_flower: int,
                 color: str) -> None:
    ...
    def show(self) -> None:
        print(f"{self.name}: {round(self.cm, 1)}cm, {self.age_plant}"
              f"days old\n Color: {self.color}")
...
class Tree(Plant):
    def __init__(self, name: str, height: float, age_tree: int,
                 trunk_diameter: float) -> None:
        super().__init__(name, height, age_tree)
        self.trunk_diameter = trunk_diameter

    def produce_shade(self) -> None:
        print(f"Tree {self.name} now produces a shade of {round(self.cm, 1)}cm"
...
class Vegetable(Plant):
    def __init__(self, name: str, cm: float, age_plant: int, harvest_season:
