...
def main() -> None:
	names:list = ['Alice', 'bob', 'Charlie', 'dylan', 'Emma', 'Gregory', 'john', 'kevin', 'Liam']
	new_list:list = []
	new_list_caps:list = []
	alchemist: dict = {}
	...
	print(f"Score average is {round(sum(alchemist.values()) / len(alchemist.keys()), 2)}\n")
	alchemist_high = {name: alchemist[name] for name in alchemist if alchemist[name] > sum(alchemist.values()) / len(alchemist.keys())}
